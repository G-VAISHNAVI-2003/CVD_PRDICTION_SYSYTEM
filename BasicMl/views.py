...
import io
import base64
from .models import user_parameter
...
def result(request):
    if request.method == 'GET':
        data = pd.read_csv(r'D:/Final_year_project/Heart/MLmodel/heartdisease.csv')
        y = data["target"]
        ...
        name = request.GET.get('name', default='User')
        user_param = user_parameter(
            Name= name,  
            Age=val1,
            Sex=val2,
            Chest_Pain_Type=val3,
            Resting_Blood_Pressure=val4,
            Serum_Cholestrol=val5,
            Fasting_Blood_Sugar=val6,
            Resting_Electrocardiographic_Results=val7,
            Maximum_Heart_Rate_Acheived=val8,
            Excercise_Induced_Angina=val9,
            Slope=val11,
            Oldpeak=val10,
            Result=result
        )
